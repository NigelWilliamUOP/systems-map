...
Nothing from Scopus beyond counts, DOIs, years and matched reference strings
reaches the output, and the corpus itself is never committed.
"""
...
def main() -> int:
    ...
    for k, weight in edge_w.items():
        ...
        edges.append({
            ...
            "plain_phrase": (f"work on {by_id[k[0]]['label'].lower()} cites "
                             f"work on {by_id[k[1]]['label'].lower()}"),
            "directed": True,
            ...
            "evidence": edge_ev[k],
            "scope_conditions": ("Counted citation only. Not influence, teaching, "
                                 "agreement, derivation or logical dependence."),
        })
    ...
    meta = {
        ...
        "built_from": "Scopus corpus; counts, DOIs and years only",
