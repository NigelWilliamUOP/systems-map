...
function tick(){
  for(let i=0;i<N.length;i++){const p=N[i];
    for(let j=i+1;j<N.length;j++){const q=N[j];
      ...
      const f=11000/d2;const ux=dx/d,uy=dy/d;
...
let frames=0;
function loop(){if(frames<620){tick();frames++}draw();requestAnimationFrame(loop)}
loop();
...
function showNode(n){
  ...
  panel.innerHTML='<h2>'+esc(n.label)+'</h2><p class="sub">'+
    ...
    '<div class="evh">Citation links</div><ul class="links">'+links.slice(0,18).map((e,i)=>
...
function showEdge(e){
  panel.innerHTML='<h2>'+esc(e.a.label)+' &rarr; '+esc(e.b.label)+'</h2>'+
    '<p class="sub">work on '+esc(e.a.label.toLowerCase())+' cites work on '+esc(e.b.label.toLowerCase())+'</p>'+
