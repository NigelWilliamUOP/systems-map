...
def main() -> int:
    ...
    for source in data.get("sources", []):
        if source.get("id") == SOURCE_ID:
            ...
            source["notes"] = (
                ...
                "sciences. Read in its current web edition, which carries 300 linked "
                "entries; the Wikimedia file recorded as archived_url is an earlier "
                "static export. Used as a comparator for coverage. Its lines carry no "
                "stated relation type, so it is not evidence for any relationship."
            )
    ...
    meta["castellani_map_entry_count"] = 300
    meta["castellani_map_already_held_count"] = 77
    meta["castellani_map_in_systemic_evolution_count"] = 148
    meta["castellani_map_gap_count"] = 133
