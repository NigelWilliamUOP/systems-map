...
- **`make validate` also failed on Python 3.12, from a clean checkout — now
  fixed.** An earlier note here recorded that it passed. That was wrong: the
  passing run began from a tree already half-patched by an aborted Python 3.11
  run, not from `main`.

  The cause was in `scripts/patch_iteration_14.py`. `patch_index()` is designed
  to be idempotent — strip any existing update dot, then re-insert exactly one
  before `</body>` — but the strip regex ended in a `(?=\n</body>)` lookahead, so
  it could only remove a dot already in that position. In the committed
  `docs/index.html` the dot sat after `</main>`, where
  `patch_adversarial_experience.py` puts it. The lookahead never matched, the
  existing dot survived, a second was appended, and `validate_iteration_14`
  rejected the page for carrying the update-thread URL twice.

  Dropping the lookahead makes the removal positional-independent. The dot is
  unchanged in markup, count and styling — one occurrence of the exact anchor the
  validator pins, still `position: fixed` at right 3px / bottom 3px with
  `aria-label="Open updates"` — so `DESIGN_AND_CONTENT_RULES.md` is satisfied.

  `make validate` now exits 0 from a clean checkout on Python 3.12, and two
  further build runs leave every output byte-identical. The build is a genuine
  fixed point, and the tree has been regenerated and committed separately so a
  later comparator overlay stays distinguishable from this drift.

  Two consequences of the regeneration need a curator decision, recorded on that
  commit: `.github/workflows/triage-site-submissions.yml` is build-generated and
  its regenerated form widens the CI triggers, and `thin_entry_count` rises from
  264 to 276 as the honest current measurement.
...
- [x] `make validate` passes on Python 3.12 from a clean checkout. The build
      blocker recorded in §3.1 has been fixed separately; this packet writes
      nothing the validators read, so it neither caused nor cleared it.
